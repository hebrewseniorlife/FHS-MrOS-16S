...
res_path <- "/n/groups/ifaromics/paul/HRPQCT_16S_Analysis/16S_results/GENUS/MrOS/"

#Load data created on #HSL RStudio
meta <- read.table(file = data_path %&% "O2_MrOS_16S_Meta.txt", 
                   sep = "\t",header = T, row.names = 1, check.names = F)

#Extract MrOS Clinic Site Variable
...
#Use the adjusted Abundance table for Subsequent analysis
#Make microbes to be the columns
adj_abd <- t(adj_abd) %>% data.frame()

#Write out adjusted abundance
write.table(adj_abd, data_path %&% "adj_O2_MrOS_16S_Abd_Genus.txt",
...
#Load adjusted abundance data created above
adj_abd <- read.table(file = data_path %&% "adj_O2_MrOS_16S_Abd_Genus.txt", 
                      sep = "\t", header = T, row.names = 1, check.names = F)
