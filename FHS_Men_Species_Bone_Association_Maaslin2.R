...
res_path <- "/n/groups/ifaromics/paul/HRPQCT_16S_Analysis/16S_results/SPECIES/FHS/men_only/"

#Load data created on #HSL RStudio
abd <- read.table(file = data_path %&% "O2_FHS_16S_Abd_Species_Men_Only.txt", 
                  sep = "\t", header = T, row.names = 1, check.names = F)
meta <- read.table(file = data_path %&% "O2_FHS_16S_Meta_Men_Only.txt", 
                   sep = "\t",header = T, row.names = 1, check.names = F)
